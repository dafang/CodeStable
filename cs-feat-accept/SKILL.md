...
**推进原则**：验收阶段要用仓库事实关闭任务，而不是用对话自述关闭任务。最终结论必须回答四个问题：原始契约是否满足、必跑命令是否在最终状态仍成立、承诺交付物是否真实落盘、这次学到的可复用知识是否进入 CodeStable 的沉淀出口。
...
3. **场景抽样复核**：对第 3 节所有可自动验证的场景尽量重跑；对截图 / 手工类标 `trust-prior-verify` 时写清依赖的证据来源
